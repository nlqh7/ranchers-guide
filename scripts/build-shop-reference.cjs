...
const resources = require('../data/build-resources.json').items;
...
function render(locale, buildingOnly=false) {
  ...
  const row = offer => {
    ...
    const resource = resources.find(i => i.id === entry.id && i.materialId);
    const seed = seeds.find(i => i.id === entry.id && i.rosterStatus === 'current-roster');
    const produceItem = produce.find(i => i.id === entry.id);
    const miscItem = miscItems.find(i => i.id === entry.id);
    const placeable = placeables.find(i => i.id === entry.id);
    const vehicle = vehicles.find(i => i.id === entry.id);
    const resourceLink = !buildingOnly && resource ? `<a class="shop-recipe-link" href="${prefix}/database/materials#${resource.materialId}">${zh?'材料资料与用途':'Material details & uses'} →</a>` : '';
    ...
    const seedLink = !buildingOnly && seed ? `<a class="shop-recipe-link" href="${prefix}/database/crops#${seed.cropId}">${zh?'查看种子与作物资料':'Seed & crop details'} →</a>` : '';
    const produceLink = !buildingOnly && produceItem ? `<a class="shop-recipe-link" href="${prefix}/database/crops#${produceItem.cropStatus === 'current-roster' ? produceItem.cropId : `seed-${produceItem.cropId}`}">${zh?'查看农产品配置':'Produce item settings'} →</a>` : '';
    const miscLink = !buildingOnly && miscItem ? `<a class="shop-recipe-link" href="${prefix}/guides/crafting-guide#tool-${miscItem.id}">${zh?'查看原生物品配置':'Native item settings'} →</a>` : '';
    const placeableLink = !buildingOnly && placeable && !hasRecipe ? `<a class="shop-recipe-link" href="${prefix}/guides/crafting-guide#placeable-${placeable.id}">${zh?'查看原生物品配置':'Native item settings'} →</a>` : '';
    const vehicleLink = !buildingOnly && vehicle ? `<a class="shop-recipe-link" href="${prefix}/guides/vehicles-transport#vehicle-${vehicle.id}">${zh?'查看车型资料':'Vehicle profile'} →</a>` : '';
    const query = `${entry.name} ${entry.zhName} ${seasons[offer.season].join(' ')} ${offer.materials.map(m=>`${item(m.id).name} ${item(m.id).zhName}`).join(' ')}`;
    const searchAttrs = buildingOnly || farmInput || resource || seed || produceItem || miscItem || placeable || vehicle ? '' : ` data-search-entry data-search-title="${esc(name(entry))}" data-search-tags="${esc(query)}" data-search-status="${zh ? '游戏构建配置' : 'Game-build configuration'} · ${esc(name(data.shops.find(s=>s.id===offer.shopId)))}"`;
  ...
  const seasonControl = `<label>${zh?'商品季节':'Listing season'}<select data-shop-season aria-describedby="shop-season-help"><option value="all">${zh?'不限季节':'Any season'}</option>${Object.entries(seasons).filter(([id])=>id!=='AllTheTime').map(([id,labels])=>`<option value="${id}">${labels[zh?1:0]}</option>`).join('')}</select></label>`;
  const body = buildingOnly ? table(data.offers.filter(o=>o.materials.length),title) : `<div class="recipe-controls"><label>${zh?'查找物品或材料':'Find an item or material'}<input type="search" data-shop-query placeholder="${zh?'例如：鸡舍、太阳能、草莓':'e.g. Coop, Solar, Strawberry'}"></label><label>${zh?'商店':'Shop'}<select data-shop-category><option value="all">${zh?'全部商店':'All shops'}</option>${data.shops.map(s=>`<option value="${s.id}">${esc(name(s))}</option>`).join('')}</select></label>${seasonControl}</div><p class="recipe-boundary" id="shop-season-help">${zh?'多个关键词用空格分隔。选择季节会保留全年条目；这是商店分组，不是种植季节。':'Separate search terms with spaces. Season filters include all-season listings; these are shop sections, not growing seasons.'}</p><p data-shop-empty hidden role="status">${zh?'没有匹配的商品。':'No matching listings.'}</p>${data.shops.map((shop,index)=>{
