...
<details class="consumable-sources"><summary>${zh?'来源与字段说明':'Sources & field definitions'}</summary><p>${zh?'非官方资料。名称逐项匹配原版双语文本，数值来自 Consumables 的 Energy/Health：恢复对应 Restor，消耗对应 Consumption。本表27条的两个消耗字段均为0；不把恢复值换算成百分比。':'Unofficial reference. Names match exact bilingual game text. Values come from Consumables Energy/Health: restore means Restor; use means Consumption. Both use fields are zero for these 27 records. Restore values are not percentages.'}</p><p>${zh?`共同配置：默认 ${esc(data.sharedConfiguration.defaultClassification)} 分类、${esc(data.sharedConfiguration.rarity)} 稀有度、TwoHandHolder 持有槽，并带可装备、可堆叠、可丢弃与可出售标志；禽肉例外标为 HUNTING。这些是来源字段，不证明玩家当前能获取、使用、出售或按该分类触发玩法。`:`Shared configuration: default ${esc(data.sharedConfiguration.defaultClassification)} classification, ${esc(data.sharedConfiguration.rarity)} rarity and TwoHandHolder slot, with equippable, stackable, droppable and sellable flags; Bird Meat is the HUNTING exception. These source fields do not prove current acquisition, use, sale or classification-driven gameplay.`}</p><p>${zh?'6种奶有饮品标记，其余21条未收录该字段，不等于否。存在配置不代表可以生食、已开放或会稳定产出；没有匹配商店条目的物品不猜购买路线。':'Six milk records carry a drink flag; the other 21 omit it rather than explicitly setting false. Configuration does not establish raw edibility, availability or reliable production. No purchase route is guessed for unmatched items.'}</p><p>Consumables / resources.assets / 12043 · ${data.build} · Steam ${data.steamBuild}. I2. <a href="${prefix}/methodology">${zh?'验证方法':'Methodology'}</a></p></details>
</section>
