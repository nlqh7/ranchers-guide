...
function renderMarker(location, locale) {
  ...
  return `                <button class="map-marker map-marker-${location.category}" type="button" style="--mx:${marker.x}%;--my:${marker.y}%" data-marker-category="${location.category}" data-marker-title="${copy.title}" data-marker-confidence="${copy.confidence}" data-marker-copy="${copy.description}" data-marker-target="${copy.target}" aria-label="${copy.ariaLabel}"><span>${copy.label}</span></button>`;
}
...
function replaceGenerated(html, data, locale) {
  ...
  const entries = locale === "en" ? data.locations.map((location) => renderEntry(location, locale)).join("\n") : "";
  const markerBody = `\n                <!-- MAP_MARKERS:START -->\n${markers}\n                <!-- MAP_MARKERS:END -->`;
  ...
  if (locale === "en") {
    ...
    if (!directoryPattern.test(output)) throw new Error("Generated English directory section missing");
