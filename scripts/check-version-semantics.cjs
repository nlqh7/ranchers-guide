...
const moneyEnglishSources = moneyEnglish.split('id="evidence"')[1]?.split('</section>')[0];
const moneyChineseSources = moneyChinese.split('id="evidence"')[1]?.split('</section>')[0];
assert.ok(moneyEnglishSources && moneyChineseSources, 'Money guides retain their dedicated source section');
assert.match(moneyEnglishSources, /retained 0\.8\.10\.842 game tutorial/, 'English selling instructions retain their actual tutorial build');
assert.match(moneyEnglishSources, /figures below remain observations of 0\.8\.10\.455/, 'English money amounts remain historical observations');
