...
problemPages.forEach((relativePath) => {
  ...
  assert.match(html, /0\.8\.10\.842/, `${relativePath} must state the current build baseline`);
...
assert.match(research, /Official fix · 0\.8\.10\.842/);
assert.match(research, /1841579228669835/);
...
assert.match(searchScript, /ranchers-search-index-v27/);
assert.match(searchScript, /ranchers-search-index-zh-v14/);
