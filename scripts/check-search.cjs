...
assert.match(fs.readFileSync(path.join(root, "assets", "js", "search.js"), "utf8"), /ranchers-search-index-zh-v28/, "Chinese search cache must invalidate the previous page set");
assert.match(fs.readFileSync(path.join(root, "assets", "js", "search.js"), "utf8"), /ranchers-search-index-v41/, "English search cache must invalidate the previous page set");
