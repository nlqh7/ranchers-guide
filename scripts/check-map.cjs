...
assert.match(mapPage, /The map opens focused at 125%; use Fit to see the complete texture\./, "English Overview must explain the focused default and Fit action");
assert.match(chineseMapPage, /地图默认以 125% 聚焦；点击“适应”查看完整底图。/, "Chinese Overview must explain the focused default and Fit action");
...
assert.match(mapPage, /data-map-stack-action="toggle"/, "English map needs an explicit all-stack expand control");
assert.match(chineseMapPage, /data-map-stack-action="toggle"/, "Chinese map needs an explicit all-stack expand control");
...
assert.match(mapPage, /data-map-stage-hud/);
assert.match(chineseMapPage, /data-map-stage-hud/);
...
assert.match(mapScript, /collisionOffsets/, "labels need fallback positions when a nearby icon would cover the text");
...
assert.match(mapScript, /nextVisibleStackMarker/);
assert.match(mapScript, /if \(stackExpanded\) \{[^}]*applyMarkerStackFan\(stack, visible\)/, "zoomed dense stacks must fan native glyphs instead of only hiding the count badge");
assert.match(mapScript, /updateMarkerStackBadges/);
assert.match(mapPage, /click the same marker again to cycle/);
assert.match(chineseMapPage, /附近还有其他锚点/);
...
assert.match(mapScript, /activeCategoryLayers\.length === markerCategories\.length && activeEvidenceFilter === "all"/, "the all-layers button must reflect both category and evidence state");
...
assert.match(mapScript, /updateMapStageHud/, "map should reflect active region, zoom and visible pin count inside the map");
